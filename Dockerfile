...
ENV \
 ...
 KIBANA_PACKAGE=kibana-oss-${KIBANA_VERSION}-linux-x86_64.tar.gz \
 KIBANA_GID=993 \
 KIBANA_UID=993
...
RUN set -x \
  && if [ "${TARGETPLATFORM}" = "linux/arm/v7" ] ; then rm -rf /usr/share/kibana/node \
  ...
  && mv node-${NODEJS_VERSION}-linux-armv6l /usr/share/kibana/node ; fi

  RUN set -x \
  && if [ "${TARGETPLATFORM}" = "linux/arm64" ] ; then rm -rf /usr/share/kibana/node \
...
RUN chmod g+s /usr/share/kibana/bin/kibana
